...
基于 `design.md` 和 `tasks.md`，将工作流模块实现拆分为可执行的最小任务。
...
1. **优先完成 1-3**（模块骨架 + 实体 + Mapper）—— 其他层都依赖
2. **然后完成 4**（API 层）—— 明确接口契约
3. **接着完成 5-6**（执行引擎）—— 核心复杂度
4. **然后完成 7**（Controller）—— HTTP 接入
5. **最后完成 8-9**（联动 + 审批）—— 扩展功能
